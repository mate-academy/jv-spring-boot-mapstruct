...
public interface StudentMapper {
    ...
    default void setSubjectIds(@MappingTarget StudentDto studentDto, Student student) {
        List<Long> listOfSubjectIds = student.getSubjects().stream()
                .map(Subject::getId)
                .toList();
        studentDto.setSubjectIds(listOfSubjectIds);
    }
    ...
    default void setSubject(@MappingTarget Student student, CreateStudentRequestDto requestDto) {
        List<Subject> listOfSubject = requestDto.subjects().stream()
                .map(Subject::new)
                .toList();
        student.setSubjects(listOfSubject);
    }
