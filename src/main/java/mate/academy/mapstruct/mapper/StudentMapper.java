...
public interface StudentMapper {
    ...
    default void setSubjectIds(@MappingTarget StudentDto studentDto, Student student) {
        List<Long> setSubjectIds = student.getSubjects().stream()
                .map(Subject::getId)
                .toList();
        studentDto.setSubjectIds(setSubjectIds);
    }
    ...
    default void setSubject(@MappingTarget Student student, CreateStudentRequestDto requestDto) {
        List<Subject> setSubject = requestDto.subjects().stream()
                .map(Subject::new)
                .toList();
        student.setSubjects(setSubject);
    }
}
